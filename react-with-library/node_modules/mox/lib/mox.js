...
var run = function(sources,options,onComplete){
    ...
    try{
        ...
        getAllFilesToParse(sources,function(sources){
            ...
            var jadeRenderer = jade.compile(file,{pretty:true});
            ...
            hammerdown({type:options.markdown}).parse(rendered).pipe(tr).pipe(concat(function(markdown){
                if(options.outputFile)
                    fs.outputFileSync(options.outputFile,markdown);

                if(onComplete instanceof Function)
                    onComplete(null,markdown);
            }));
        });
    }catch(exception){
        if(onComplete)
